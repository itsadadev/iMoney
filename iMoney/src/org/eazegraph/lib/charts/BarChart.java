...
public class BarChart extends BaseBarChart {
	 ...
	 * 
	 * @param _Bar
	 *            The BarModel which will be added to the chart.
	 */
	...
	/**
	 * Adds a new list of {@link org.eazegraph.lib.models.BarModel} to the
	 * BarChart.
	 * 
	 * @param _List
	 *            The BarModel list which will be added to the chart.
	 */
	 ...
	 * called, when the view dimensions has changed.
	 */
	@Override
	...
	protected void calculateBounds(float _Width, float _Margin) {
		...
		int valuePadding = mShowValues ? (int) mValuePaint.getTextSize()
				+ mValueDistance : 0;
		...
		for (BarModel model : mData) {
			...
			last += _Margin / 2;
			model.setBarBounds(new RectF(last, mGraphHeight - height, last
					+ _Width, mGraphHeight));
			model.setLegendBounds(new RectF(last, 0, last + _Width,
					mLegendHeight));
			last += _Width + (_Margin / 2);
		}

		Utils.calculateLegendInformation(mData, 0, mContentRect.width(),
				mLegendPaint);
	}
	...
	protected void drawBars(Canvas _Canvas) {
		...
		for (BarModel model : mData) {
			...
			_Canvas.drawRect(bounds.left, bounds.bottom
					- (bounds.height() * mRevealValue), bounds.right,
					bounds.bottom, mGraphPaint);

			if (mShowValues) {
				_Canvas.drawText(
						Utils.getFloatString(model.getValue(), mShowDecimal),
						model.getLegendBounds().centerX(), bounds.bottom
								- (bounds.height() * mRevealValue)
								- mValueDistance, mValuePaint);
			}
		}
	}
