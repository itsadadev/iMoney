...
/**
 * The abstract class for every type of bar chart, which handles the general
 * calculation for the bars.
 */
@SuppressLint("NewApi")
public abstract class BaseBarChart extends BaseChart {
	...
	public BaseBarChart(Context context, AttributeSet attrs) {
		...
		try {
			...
			// mShowValues = a.getBoolean(R.styleable.BaseBarChart_egShowValues,
			// DEF_SHOW_VALUES);
			...
			// mFixedBarWidth =
			// a.getBoolean(R.styleable.BaseBarChart_egFixedBarWidth,
			// DEF_FIXED_BAR_WIDTH);
			// mScrollEnabled =
			// a.getBoolean(R.styleable.BaseBarChart_egEnableScroll,
			// DEF_SCROLL_ENABLED);
			// mVisibleBars = a.getInt(R.styleable.BaseBarChart_egVisibleBars,
			// DEF_VISIBLE_BARS);
...
//			a.recycle();
	 ...
	 * 
	 * @param _listener
	 *            The listener which will be set.
	 */
	...
	/**
	 * Returns the bar margin, which is set by user if the bar widths are
	 ...
	 * 
	 * @param _showValues
	 *            true to show values in the graph.
	 */
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mAvailableScreenSize = this instanceof VerticalBarChart ? mGraphHeight
				: mGraphWidth;
	...
	/**
	 * Calculates the bar width and bar margin based on the _DataSize and
	 * settings and starts the boundary calculation in child classes.
	 * 
	 * @param _DataSize
	 *            Amount of data sets
	 */
	protected void calculateBarPositions(int _DataSize) {
		...
		if (!mFixedBarWidth) {
			// calculate the bar width if the bars should be dynamically
			// displayed
			...
			float remainingScreenSize = mAvailableScreenSize
					- cumulatedBarWidths;
	...
	private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2,
			...
			if (mCurrentViewport.left + distanceX > mContentRect.left
					&& mCurrentViewport.right + distanceX < mContentRect.right) {
				mCurrentViewport.left += distanceX;
				mCurrentViewport.right += distanceX;
			}

			if (mCurrentViewport.top + distanceY > mContentRect.top
					&& mCurrentViewport.bottom + distanceY < mContentRect.bottom) {
	...
	private void fling(int velocityX, int velocityY) {
		...
		mScroller.fling((int) mCurrentViewport.left,
				(int) mCurrentViewport.top, velocityX, velocityY, 0,
				mContentRect.width() - mGraphWidth, 0, mContentRect.height()
						- mGraphHeight);
	...
	private void tickScrollAnimation() {
		if (!mScroller.isFinished()) {
			...
			if (currX > mContentRect.left
					&& currX + mGraphWidth < mContentRect.right) {
				mCurrentViewport.left = currX;
				mCurrentViewport.right = currX + mGraphWidth;
			}

			if (currY > mContentRect.top
					&& currY + mGraphHeight < mContentRect.bottom) {
	 ...
	 * 
	 * @param _Canvas
	 *            The canvas object of the graph view.
	 */
	protected abstract void drawBars(Canvas _Canvas);

	/**
	 * Returns the list of data sets which hold the information about the legend
	 * boundaries and text.
	 * 
	...
	// All float values are dp values and will be converted into px values in
	// the constructor
